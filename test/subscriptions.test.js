...
describe('AWS SNS Subscriptions Service', () => {
  beforeAll(() => {
    ...
    AWS.mock('SNS', 'unsubscribe', { ResponseMetadata: { RequestId: '361d86ce-a8cf-51bf-b7c5-d3776795f22a' } });

    app = express(feathers())
      .use('/sns/subscriptions', new Subscriptions({
        region: 'us-east-1',
  ...
  test('returns a list of the subscriptions to a specific topic.', async () => {
    const result = await app.service('sns/subscriptions').get({ TopicArn: 'arn:aws:sns:us-east-1:658710424301:MySNSTopic' });
    // console.log(result);
  ...
  test('Deletes a subscription', async () => {
    const result = await app.service('sns/subscriptions').remove({
      SubscriptionArn: 'arn:aws:sns:us-east-1:658710424301:MySNSTopic:2702ec54-c896-4b17-927f-b04380f03bf2'
    });
    expect(result).toStrictEqual({ ResponseMetadata: { RequestId: '361d86ce-a8cf-51bf-b7c5-d3776795f22a' } });
  });
});
